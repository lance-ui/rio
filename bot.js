...
const { convertToGothic,  translate, uploadImgbb, mssg, downloadFile, translateAPI, getStreamsFromAttachment, getExtFromUrl, getStreamFromURL, shortenURL } = require('./utils/utils.js');
...
global.botname = global.config.botname;
...
global.getStreamsFromAttachment = getStreamsFromAttachment;
global.getExtFromUrl = getExtFromUrl;
global.getStreamFromURL = getStreamFromURL;
global.shortenURL = shortenURL;
global.translateAPI = translateAPI;
global.downloadFile = downloadFile;
global.uploadImgbb = uploadImgbb;
global.translate =  translate;
global.timezone = global.config.timezone;
...
const initialize = async () => {
    ...
    await loadFiles('./scripts/cmds', 'commands');
    await loadFiles('./scripts/events', 'events');

    if (global.autoLoad) {
        watchFiles('./scripts/cmds', 'commands');
        watchFiles('./scripts/events', 'events');
    }
};
    ...
    fs.readFile('appstate.json', 'utf8', (err, data) => {
        ...
        login(options, (loginError, api) => {
            ...
            process.on('unhandledRejection', (err, p) => {
                systemLog(`Unhandled Rejection: ${err}`);
            });
        });
    });
};
